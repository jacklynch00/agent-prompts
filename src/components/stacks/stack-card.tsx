		...
		<Card className='h-full hover:shadow-lg transition-shadow duration-200'>
				...
				<div className='flex items-start justify-between'>
					<div className='space-y-2'>
						...
						<div className='flex items-center gap-2 flex-wrap'>
							<Badge className={getCategoryColor(stack.category)}>{stack.category}</Badge>
						</div>
					</div>
				</div>
				<CardDescription className={showFullDescription ? '' : 'line-clamp-2'}>{stack.description}</CardDescription>
			</CardHeader>
			...
			<CardContent className='space-y-4'>
				...
				<div className='flex items-center gap-1 text-xs text-muted-foreground'>
					...
					<span>Updated {new Date(stack.metadata.updatedAt).toLocaleDateString()}</span>
				</div>

				{/* Action button */}
				<div className='pt-2'>
					<Button asChild className='w-full'>
						<Link href={`/stacks/${stack.id}`}>View Stack & Agents</Link>
					</Button>
				</div>
			</CardContent>
		</Card>
