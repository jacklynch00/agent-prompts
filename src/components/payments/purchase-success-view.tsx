				...
				<div className='max-w-md w-full px-4'>
					<Card className='text-center'>
						<CardContent className='pt-6'>
							...
							<p className='text-muted-foreground mb-4'>{error || 'Unable to verify your purchase. Please contact support.'}</p>
							<Button asChild>
								<Link href='/'>
			...
			<div className='max-w-2xl mx-auto px-4 py-12'>
				<Card className='text-center'>
					<CardHeader>
						...
						<CardTitle className='text-2xl font-bold mb-2'>{isSuccessful ? 'Purchase Successful!' : 'Processing Payment...'}</CardTitle>
						<div className='text-muted-foreground'>
							{isSuccessful ? 'You now have premium access to all AI agents!' : 'Your payment is being processed. Please wait a moment.'}
						</div>
					</CardHeader>
					...
					<CardContent className='space-y-6'>
						...
						<div className='bg-muted rounded-lg p-4'>
							...
							<div className='space-y-2 text-sm'>
								...
								<div className='flex justify-between'>
									<span>Date:</span>
									<span>{new Date(purchaseData.createdAt).toLocaleDateString()}</span>
								</div>
								<div className='flex justify-between'>
									<span>Status:</span>
									<span className={`font-medium ${isSuccessful ? 'text-green-600' : 'text-yellow-600'}`}>{isSuccessful ? 'Completed' : 'Processing'}</span>
								</div>
							</div>
						</div>
						...
						{isSuccessful && !purchaseData.isLoggedIn && !purchaseData.hasAccount && !showAccountCreation && !accountCreated && (
							<div className='text-center'>
								...
								<p className='text-muted-foreground mb-4'>Create an account to manage your premium access and stay updated with new releases.</p>
								<Button onClick={() => setShowAccountCreation(true)} size='lg'>
									<UserPlus className='h-4 w-4 mr-2' />
									Create Account
								</Button>
								<p className='text-xs text-muted-foreground mt-2'>Optional - you already have access to all premium content</p>
							</div>
						)}
						...
						{isSuccessful && !purchaseData.isLoggedIn && purchaseData.hasAccount && (
							<div className='text-center'>
								...
								<p className='text-muted-foreground mb-4'>We found an existing account with your email. Sign in to access your premium content.</p>
								<Button asChild size='lg'>
									<Link href={`/auth/signin?redirect=${encodeURIComponent('/dashboard')}`}>Sign In to Your Account</Link>
								</Button>
							</div>
						)}
						...
						{isSuccessful && (
							<div className='bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4'>
								<div className='flex items-center gap-2 mb-3'>
									...
									<h3 className='font-semibold text-yellow-800 dark:text-yellow-200'>Premium Benefits Unlocked</h3>
								</div>
						...
						<div className='text-xs text-muted-foreground border-t pt-4'>
							<p>Questions about your purchase? Contact us at support@getagentprompts.com</p>
							<p className='mt-1'>
								Checkout ID: <code className='bg-muted px-1 rounded text-xs'>{checkoutId}</code>
							</p>
						</div>
					</CardContent>
				</Card>
